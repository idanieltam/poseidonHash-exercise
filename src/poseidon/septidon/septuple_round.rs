...
impl SeptupleRoundChip {
    pub fn configure<F: CachedConstants> (cs: &mut ConstraintSystem<F>, q: Expression<F>) -> (Self,LoopBody<F>) {
        ...
        let (input_state, next_state) = query(cs, |meta| {
            ([
                input[0].query(meta, 0), // read via first_sbox.output_expr
        ...
        let output = {
            ...
            // The input state is not constrained here but constrained by another chip (TransitionRoundChip) before septuple round.
            ...
            cs.create_gate("septuple_round",|meta|{
                ...
                for sbox_to_check in &chip.following_sboxes {
                    ...
                    // validate the sbox input, so we can use next_sbox.output_expr
    ...
    fn partial_round_expr<F:CachedConstants>(
        ...
        let sbox_out = [
            sbox.output_expr(meta),input[1].clone(),input[2].clone(),
        ];
        matmul::expr(mds(), sbox_out)
    }

    pub fn assign<F:CachedConstants>(
        &self, region: &mut Region<'_, F>, input: &[Value<F>;3],
        round_constants: &[F], offset: usize,
    ) -> Result<[Value<F>;3],Error> {
        // Assign the first non-sbox cells
        for i in 0..2 {
        ...
        // Define a helper FnMut to assign a partial round
        let mut assign_partial_round = |i: usize, sbox: &SBox| -> Result<(),Error> {
            ...
            state = split_value(join_values(state).map(|s| matmul::value(mds(), s)))?;
            Ok(())
        };
