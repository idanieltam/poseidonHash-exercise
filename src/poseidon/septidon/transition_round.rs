...
use super::util::{join_values, matmul, split_value};
...
impl TransitionRoundChip {
    pub fn configure<F: CachedConstants> (
        ...
        cs.create_gate("transition round", |meta|{
            // The input cells are related to the signal
            ...
            // Get the next_state from the point of the view of the signal
            let next_state = [
        ...
        // Return an expression of the state after the first partial round given the previous state
        // Implemented with a helper cell to make degree <= 5
        ...
        fn round_constant<F: CachedConstants>() -> F {
            round_constant(4)[0] // why index is 4?
        }
        ...
        fn first_partial_round<F:CachedConstants>(
            input: &[Value<F>;3],
        )->[Value<F>;3] {
            let sbox_out = [
        ...
        pub fn assign_first_partial_round<F: CachedConstants> (
            &self,
            ...
            self.helper_cell().assign(region, middle_break_offset, Value::known(F::zero()))?;
            Ok(output)
        }
        ...
        pub fn assign_final_state<F:CachedConstants>(
            ...
            self.helper_cell().assign(region, final_break_offset, Value::known(F::zero()))?;
            Ok(())
        }
}
